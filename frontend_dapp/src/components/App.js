...
const App = () => {
  ...
  const privateKey =
    "0x8aa1a15d67c8a56c7b16acc2184784ef1dc82d481ecad77cdc67d81811f03310";
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // );
        const contract = new web3.eth.Contract(Wallets.abi, contractAddress, {
          from: accounts[0],
        });
  ...
  const sendDaiToWallet = async (amount) => {
    ...
    let transferResult = await daiContract.methods
      .transfer(contractAddress, web3.utils.toHex(amount * Math.pow(10, 18)))
      .send(fromMyWallet);
  ...
  const supplyToCompound = async (amount) => {
    ...
    let supplyResult = await contract.methods
      .supplyDAIToCompound(
        daiMainNetAddress,
        compoundCDaiContractAddress,
        web3.utils.toHex(amount * Math.pow(10, 18))
      )
      .send(fromMyWallet);
  ...
  const redeemCDAITokens = async (amount) => {
    ...
    let redeemResult = await contract.methods
      .redeemCDai(amountToRedeem, redeemType, compoundCDaiContractAddress)
      .send(fromMyWallet);
    ...
    <div className="rootSectionMenu">
      <AccountsContext.Provider value={(accounts, setAccounts)}>
        <ConnectedContext.Provider value={(isConnected, setIsConnected)}>
          <Compound />
          <Route path="/deposit">
            <DepositDai deposit={sendDaiToWallet} />
          </Route>
          <Route path="/dai-balance">
            <DaiBalancePage
              checkBalance={checkDaiBalance}
              daiBalance={daiBalance}
            />
          </Route>
          <Route path="/send-to-compound">
            <SupplyToCompoundPage deposit={supplyToCompound} />
          </Route>
          <Route path="/compound-balance">
            <BalanceInCompoundPage
              checkBalance={balanceInCompound}
              cDaiBalance={cDaiBalance}
            />
          </Route>
          <Route path="/redeem-cdai">
            <RedeemTokensPage deposit={redeemCDAITokens} />
          </Route>
